...
contract Token {
    ...
    function transfer(address target, uint256 _value) public returns (bool success);
    function transferFrom(address from, address target, uint256 _value) public returns (bool success);
...
contract StandardToken is Token, Controlled {
    ...
    function transferFrom(address from, address target, uint256 value) public transferAllowed(msg.sender) validAddress(target) returns (bool success) {
        require(value > 0);
        if (balances[from] >= value && allowed[from][msg.sender] >= value && balances[target] + value > balances[target]) {
            balances[target] += value;
            balances[from] -= value;
