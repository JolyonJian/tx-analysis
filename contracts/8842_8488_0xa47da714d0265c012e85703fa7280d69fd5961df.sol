...
contract Swarm is ERC2981, ERC721AQueryable, Ownable {
    ...
    function mint(uint32 amount) external payable {
        require(_started, "Gas is not ready");

        uint32 publicMinted = _publicMinted();
        uint32 publicSupply = _publicSupply();
        require(amount + publicMinted <= _publicSupply(), "Out of swarm gas");

        uint32 minted = uint32(_numberMinted(msg.sender));
        require(amount + minted <= _walletLimit, "You got enough swarm gas");
