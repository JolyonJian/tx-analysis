...
contract BFHZELGatewayV1 is Operatable {
    ...
    uint8 constant PURCHASE_ZELUP20 = 2;
    uint8 constant PURCHASE_REGULAR = 3;
    uint8 constant PURCHASE_ETH_BACK_ZELUP20 = 4;
    ...
    function setCampaign(
        ...
        onlyOperator()
    {
        campaign = Campaign(_since, _until, _purchaseType, _subPurchaseType);
        emit CampaignUpdated(_since, _until, _purchaseType, _subPurchaseType);
    }
        ...
        whenNotPaused()
    {
        ...
        (uint8 purchaseType, uint8 subPurchaseType) = getPurchaseType(block.number);
        uint256 netValue = msg.value;
        uint256 referralValue = 0;

        if (purchaseType == PURCHASE_ETH_BACK || purchaseType == PURCHASE_ETH_BACK_ZELUP20) {
    ...
    function getPurchaseType(uint256 _block) public view returns (uint8, uint8) {
        ...
        return (campaign.purchaseType, campaign.subPurchaseType);
    }
    ...
    function encodeData(address _sender, address _referrer, uint256 _referralBasisPoint) public pure returns (bytes32) {
        return keccak256(
            abi.encode(
                _sender,
                _referrer,
