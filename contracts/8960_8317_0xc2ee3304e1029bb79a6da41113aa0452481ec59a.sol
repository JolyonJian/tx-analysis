...
contract NoodleChef is Ownable {
    using SafeMath for uint256;
    ...
    uint256 public minimumNoodlePerBlock = 125 ether; // Start at 1000, halve 3 times, 500 > 250 > 125.
    ...
    event Halve(uint256 newNoodlePerBlock, uint256 nextHalveBlockNumber);
    ...
    function doHalvingCheck(bool _withUpdate) public {
        ...
        uint256 newNoodlePerBlock = noodlePerBlock.div(2);
        if (newNoodlePerBlock >= minimumNoodlePerBlock) {
            noodlePerBlock = newNoodlePerBlock;
            lastHalveBlock = block.number;
            emit Halve(newNoodlePerBlock, block.number + halvePeriod);
    ...
    function add(
        ...
        require(
            ...
            "NoodleChef: LP Token Address already exists in pool"
        );
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function migrate(uint256 _pid) public onlyOwner {
        require(
            ...
            "NoodleChef: Address of migrator is null"
        );
        ...
        require(
            ...
            "NoodleChef: New LP Token Address already exists in pool"
        );
        require(
            bal == newLpToken.balanceOf(address(this)),
            "NoodleChef: New LP Token balance incorrect"
        );
        ...
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 noodleReward = blockPassed
                .mul(noodlePerBlock)
                .mul(pool.allocPoint)
                .div(totalAllocPoint);
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 noodleReward = blockPassed
            ...
            .mul(pool.allocPoint)
            .div(totalAllocPoint);
        noodle.mint(devaddr, noodleReward.div(16)); // 6.25%
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        require(
            user.amount >= _amount,
            "NoodleChef: Insufficient Amount to withdraw"
        );
        updatePool(_pid);
    ...
    function dev(address _devaddr) public {
        require(
            msg.sender == devaddr,
            "NoodleChef: Sender is not the developer"
        );
        devaddr = _devaddr;
    }
}